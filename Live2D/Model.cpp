...
csmByte* CreateBuffer(const csmChar* path, csmSizeInt* size) {
  return Utilities::LoadFileAsBytes(path, size);
}

void DeleteBuffer(csmByte* buffer, const csmChar* path = "") {
  Utilities::ReleaseBytes(buffer);
}
}  // namespace
...
void Model::Update() {
  const csmFloat32 deltaTimeSeconds = Utilities::GetDeltaTime();
...
CubismMotionQueueEntryHandle Model::StartRandomMotion(
...
void Model::Draw(const CubismMatrix44& matrix) {
  if (_model == NULL) {
    return;
  }
  CubismMatrix44 viewMatrix = matrix;
  viewMatrix.MultiplyByMatrix(_modelMatrix);
  GetRenderer<Rendering::CubismRenderer_OpenGLES2>()->SetMvpMatrix(&viewMatrix);
  GetRenderer<Rendering::CubismRenderer_OpenGLES2>()->DrawModel();
}

csmBool Model::MouseTest(csmFloat32 x, csmFloat32 y) {
  if (_opacity < 1) {
    return false;
  }
  ...
  for (csmInt32 i = 0; i < count; i++) {
    ...
    if (IsHit(drawID, x, y)) return true;
  }
  return false;
}
...
void Model::SetupTextures() {
  for (csmInt32 textureNumber = 0;
       textureNumber < _modelSetting->GetTextureCount(); textureNumber++) {
    if (strcmp(_modelSetting->GetTextureFileName(textureNumber), "") == 0) {
      continue;
    }
...
Rendering::CubismOffscreenFrame_OpenGLES2& Model::GetRenderBuffer() {
